...
| HTTP bootstrap authorization and session/CSRF values | Authenticate a synthetic lab principal, establish a bounded session, and protect mutations | Request headers plus process-local session registry | Bootstrap header exists for login only; opaque session ID is cookie-bound; CSRF value is returned to the client | Idle/absolute TTL or logout | Automatic expiry or explicit revocation | Must not enter errors, lab state, audit, logs, or simulator persistence |
